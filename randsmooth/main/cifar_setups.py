...
from randsmooth.base.cifarnet import CifarNet
...
def pretrain_noise_fetch(noise_type, noise_sigma, params, stability=False):
    dirname = 'cifar_stability' if stability else 'cifar'
    filename = f'cifar_{noise_type}_{sigma_string(noise_sigma)}.pt'
...
def main(params):
    ...
    base_factories = [
        BaseFactory(
            name='base_l1', base_class=CifarNet, epochs=0,
            params={'load_class': WideResNet, 'load_path': l1_path}
        ),
        BaseFactory(
            name='base_linf', base_class=CifarNet, epochs=0,
            params={'load_class': WideResNet, 'load_path': linf_path}
        ),
        BaseFactory(
            name='base_reg', base_class=CifarNet, epochs=0,
            params={'load_class': WideResNet, 'load_path': main_path}
        ),
        BaseFactory(
            name='base_project', base_class=CifarNet, epochs=50,
            params={'load_class': WideResNet, 'load_path': main_path,
                    'project_n': 450, 'project_sigma': sigma}
        ),
    ]
...
def project_sweep(params):
    ...
    for project_n in [100, 200, 300, 450, 600, 1000]:
        base_factories.append(
            BaseFactory(
                name=f'base_project{project_n}', base_class=CifarNet, epochs=50,
                params={'load_class': WideResNet, 'load_path': main_path,
...
def rs4a_l1_sweep(params):
    ...
    for sigma in [0.25, 0.50, 0.75, 1.00]:
        ...
        base_factories.append(
            BaseFactory(
                name=model_name, base_class=CifarNet, epochs=0,
...
def attack(params):
    ...
    base_factory = BaseFactory(
        name='base', base_class=CifarNet, epochs=0,
        params={'load_class': WideResNet, 'load_path': main_path}
    )

    attack_space = params.dataset.pca_components[620:].to(torch_utils.device())
    attack_factories = []
